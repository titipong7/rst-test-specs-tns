...
# All three cases are spec'd as ``Implemented: false`` in cases.yaml,
# but fixtures are kept for matrix continuity (same pattern as rde-12).
...
def test_every_active_dnssec_ops_case_has_at_least_one_fixture(case_id: str) -> None:
...
def test_dnssec_ops_csv_fixtures_parse(path: Path | None) -> None:
    if path is None:
...
def test_dnssec_ops_pgp_armored_headers_present(path: Path | None) -> None:
    if path is None:
