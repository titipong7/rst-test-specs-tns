...
ACTIVE_CASES: tuple[str, ...] = ("idn-01", "idn-02")
...
def test_every_active_idn_case_has_at_least_one_fixture(case_id: str) -> None:
...
def test_idn_csv_fixtures_parse(path: Path | None) -> None:
    if path is None:
...
def test_idn_pgp_armored_headers_present(path: Path | None) -> None:
    if path is None:
