...
| ----------- | ------------------------------------------------------- | ------------------------------------------------------ | ---------------------------------------------------------------------------------- |
| `dnssec-91` | `91-signing-algorithm/ds-records.success.json`          | `91-signing-algorithm/ds-records.failure.json`         | Failure uses algorithm `5` (RSASHA1) which is below the `>= 8` minimum.            |
| `dnssec-92` | `92-ds-digest-algorithm/ds-records.success.json`        | `92-ds-digest-algorithm/ds-records.failure.json`       | Failure mixes `digestType=1` (SHA-1) and `digestType=12` (GOST), both forbidden.   |
| `dnssec-93` | `93-nsec3-iterations/nsec3param.success.json`           | `93-nsec3-iterations/nsec3param.failure.json`          | Happy enforces `iterations=0` and empty salt; failure uses non-zero/with salt.     |

`tests/dnssec/test_dnssec_fixtures_present.py` enforces presence and
JSON validity for every active spec case.
