...
| 5  | DNSSEC-Ops re-aligned to the flat EPP layout in a follow-up commit on the same branch. | n/a      | `internal-rst-checker/fixtures/dnssec-ops/{01,02,03}-*-{success,failure}.json` + scoped `01-zsk-rollover-tsig.env.example`. Guard test refactored to `ACTIVE_CASES = ("01","02","03")` with glob-based presence check. | None — closes the only documented "low" gap.   |
...
- **High:** 0
- **Medium:** 0
- **Low:** 3 (#3, #4, #6 — all explained and either intentional or pre-existing; finding #5 was closed by the DNSSEC-Ops follow-up commit)
...
> Re-aligns all 8 non-EPP suites (DNS, DNSSEC, DNSSEC-Ops, RDE, RDAP,
> SRSGW, IDN, Integration) to the flat EPP fixture layout
